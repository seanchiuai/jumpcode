...
A template workspace for the cockpit. Copy this directory to `workspaces/<name>/`,
...
> charters under `<workspace_root>/.agent-cockpit/roles/` (in that repo's git). This repo
> ships the engine plus an example **base set** of roles and this example config.

## How roles are resolved

`start-webapp` (or `COCKPIT_WORKSPACE=<name> start-webapp`) calls
`cockpit roles discover --workspace <name> --json`. Discovery:

1. starts from the central base set in `$COCKPIT_HOME/roles/*.md`;
2. overlays any repo-local prompts in `<workspace_root>/.agent-cockpit/roles/*.md`,
...
Projects and tasks live in **Linear** (ADR 0003); the cockpit keeps no local task state.

## Launch

```bash
# from the cockpit's parent dir
COCKPIT_WORKSPACE=example ./.agent-cockpit/bin/start-webapp
```
