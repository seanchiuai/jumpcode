...
`cockpit roles discover --workspace <name> --json`. For this example base set that is:
...
repo-specific system prompts live in the project repo while the cockpit binaries/state
stay shared. Each agent reorients from durable sources — its charter, the shared protocol,
Linear, and the dispatch log (`$COCKPIT_BIN/dispatch log 40`) — and waits to be woken by a
