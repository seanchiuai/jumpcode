...
Projects and tasks live in **Linear**, not in the cockpit. A project is a Linear project; a task is a Linear issue. The orchestrator (and Hermes) read/write them via Linear. The cockpit deliberately keeps **no** local project/task/run registry.
...
- **Completion is informal.** There is no cockpit "project done" event. Work ends when the Human closes the live workspace window (see workspace launch modes). Linear holds whatever status is meaningful.
