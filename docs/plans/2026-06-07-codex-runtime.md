...
**Goal:** Let any cockpit role run **Codex** instead of Claude Code, chosen per-role in `workspace.json`, without changing the dispatch/wake/Linear layers.

**Architecture:** Add a per-role `role_runtimes` map (default `claude`) read by the launcher; an inline, data-shaped runtime descriptor that maps a runtime name → `{program, markers}`; tag each pane with a `@cockpit_runtime` tmux option (mirroring `@cockpit_role`); and make `pane_state()` runtime-keyed so health classifies Codex panes correctly. Everything already runtime-agnostic (dispatch log, `wake_pane`, `@cockpit_role`, hub-and-spoke) is untouched.

**Tech Stack:** Python 3 stdlib (`bin/cockpit`), bash (`bin/start-webapp`), tmux, `unittest`. Design doc: `docs/plans/2026-06-07-codex-runtime-design.md`.

**Conventions you must follow (read before starting):**
- Tests run with `python3 -m unittest discover -s tests -v` from `.agent-cockpit/` (there is no pytest; `bin/cockpit` has no `.py` extension, so tests import it via `SourceFileLoader` — see any `tests/test_*.py` header).
- Git is **local-only**, scoped to `.agent-cockpit/`. Never push. Commit messages end with the `Co-Authored-By` trailer used in `git log`.
- Run every command from `/Users/seanchiu/Desktop/workspace-macbook/.agent-cockpit`.
...
**Files:**
- Modify: `bin/cockpit:286-298` (the `_BUSY_MARKER`/`_WAITING_MARKERS` globals and `pane_state`)
...
Replace `bin/cockpit:286-298` with:
...
```bash
git add bin/cockpit tests/test_health.py
...
### Task 2: Wire `@cockpit_runtime` into `cmd_health`

**Files:**
- Modify: `bin/cockpit:355-382` (the `list-panes` format string in `cmd_health` and the per-line parse loop)

**Step 1: Extend the tmux format string**

In `cmd_health`, change the `list-panes -F` format (currently `bin/cockpit:359-360`) from:

```python
                ["tmux", "list-panes", "-t", session, "-F",
                 "#{pane_id}\t#{@cockpit_role}\t#{pane_current_command}\t#{pane_dead}"],
```
...
In the parse loop (`bin/cockpit:368-382`), the current guard `if len(parts) < 4: continue` stays. Read the optional fifth field and pass it to `pane_state`:
...
In the text-output loop (`bin/cockpit:412-416`), include the runtime so a mixed team is visible at a glance:
...
Expected: each row now shows a runtime column (`claude` for all current panes, since none are codex yet) and still classifies state. If the session isn't running, this prints the graceful "no cockpit panes" message — that's fine.

**Step 5: Commit**

```bash
git add bin/cockpit
git commit -m "feat(health): read @cockpit_runtime per pane; show runtime column
...
for role in order:
    ...
    print("%s\t%s\t%s\t%s" % (role, prompts.get(role, ".agent-cockpit/roles/%s.md" % role), em, rt))
```
...
### Task 4: Runtime descriptor, program selection, preflight, and `@cockpit_runtime` tagging
...
- Orchestrator (`bin/start-webapp:105`): append `"$ORCH_RUNTIME"`:
  ```bash
  ORCH=$(tmux new-session -d -P -F '#{pane_id}' -x "${COCKPIT_COLS:-260}" -y "${COCKPIT_ROWS:-72}" -s "$SESSION" -n roles -c "$ROOT" "$(agent_cmd orchestrator "$ORCH_PROMPT" "$ORCH_RUNTIME")")
  ```
...
**Step 5: Tag each pane with `@cockpit_runtime`**

In the lead label loop (`bin/start-webapp:144-151`), after the `@cockpit_role` set-option, add:

```bash
  tmux set-option -pt "$p" @cockpit_runtime "${LEAD_RUNTIMES[$i]}"
```

For the orchestrator (`bin/start-webapp:153-156`), after its `@cockpit_role`:

```bash
tmux set-option -pt "$ORCH" @cockpit_runtime "$ORCH_RUNTIME"
```

**Step 6: Headless launcher test — `@cockpit_runtime` is set per pane and unknown→fail**
...
JSON
mkdir -p "workspaces/_rttest" && cp /tmp/cockpit-rt-test.json "workspaces/_rttest/workspace.json"

CLAUDE_BIN=cat CODEX_BIN=cat COCKPIT_WORKSPACE=_rttest COCKPIT_TMUX_SESSION=cockpit-rttest \
  COCKPIT_OPEN_ITERM=0 COCKPIT_CLAUDE_START_DELAY=0 ./bin/start-webapp >/dev/null 2>&1 || true
sleep 1
echo "=== @cockpit_runtime per pane ==="
tmux list-panes -t cockpit-rttest:roles -F '#{@cockpit_role} -> #{@cockpit_runtime}'
tmux kill-session -t cockpit-rttest 2>/dev/null || true
rm -rf "workspaces/_rttest" /tmp/cockpit-rt-test.json
```
...
CLAUDE_BIN=cat COCKPIT_WORKSPACE=_rttest COCKPIT_TMUX_SESSION=cockpit-rttest2 \
  COCKPIT_OPEN_ITERM=0 ./bin/start-webapp; echo "exit=$?"
tmux kill-session -t cockpit-rttest2 2>/dev/null || true
...
git commit -m "feat(launcher): per-runtime program selection + preflight + @cockpit_runtime tag
...
Run: `tmux list-panes -t macbook-webapp:roles -F '#{@cockpit_role} #{@cockpit_runtime} #{pane_current_command}'`
...
Expected: the codex lead shows `codex working`. **Read its pane** (`tmux capture-pane -p -t <pane>`) and confirm the busy line contains `esc to interrupt`. If the actual string differs, update `_RUNTIME_MARKERS["codex"]["busy"]` in `bin/cockpit`, re-run `python3 -m unittest tests.test_health`, and commit that fix.
...
- A mixed `role_runtimes` launches the right program per pane, tags `@cockpit_runtime`, and refuses unknown runtimes / missing binaries at preflight.
