# Agent Cockpit Rebuild Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Rebuild the agent cockpit to match the grilled design — a working dispatch (live-wake + durable-log) system over visible orchestrator/team-lead panes, with projects/tasks delegated entirely to Linear.

**Architecture:** Slim the Python CLI down to *dispatch* only (retire the local run/task/project/report registry — Linear is the system of record per ADR 0003). Make the wake mechanism actually work by targeting panes via a machine-readable `@cockpit_role` tmux option instead of the Claude-overwritten pane title. Restructure role prompts into thin charters + one shared protocol. Always launch fresh (ADR 0004); agents orient from Linear + the dispatch log.

**Tech Stack:** Python 3 (stdlib only), bash, tmux, Claude Code CLI, Linear MCP (used by the agents, not the cockpit).

**Authoritative design refs (read before starting):**
- `.agent-cockpit/CONTEXT.md` (glossary)
- `.agent-cockpit/docs/adr/0001`–`0004`
...
- **The cockpit never calls Linear.** Linear is the *agents'* responsibility via their Claude Code Linear MCP. No Linear code lands in the Python CLI. "Wiring Linear" = MCP availability for panes + charter instructions.
...
git init
printf '%s\n' '.agent-cockpit/state/' '.agent-cockpit/shared/' '__pycache__/' '*.pyc' > .gitignore
git add -A && git commit -m "chore: snapshot cockpit before grilled-design rebuild"
```

**Step 3 (if no):** Note that "Commit" steps below are skipped; rely on manual backups. Make one now:
```bash
cp -r .agent-cockpit ".agent-cockpit.bak.20260606"
```
...
**Files:**
- Modify: `.agent-cockpit/bin/start-webapp` (the `tmux set-option ... @role` block)

**Why:** Wake must target a stable id. Claude overwrites pane *titles*; the pretty `@role` label is for humans. Add a parallel machine id `@cockpit_role` = canonical role (`orchestrator`, `frontend-lead`, `backend-lead`, `qa-lead`).
...
```bash
bash -n .agent-cockpit/bin/start-webapp
```
Expected: no output (valid).

**Step 3: Commit**
```bash
git add .agent-cockpit/bin/start-webapp && git commit -m "feat(wake): tag panes with machine-readable @cockpit_role"
```
...
**Files:**
- Create: `.agent-cockpit/bin/cockpit` — add `resolve_pane(panes_text, role)` function
- Test: `.agent-cockpit/tests/test_wake.py`

**Step 1: Write the failing test**
```python
# .agent-cockpit/tests/test_wake.py
import sys, unittest
from pathlib import Path
sys.path.insert(0, str(Path(__file__).resolve().parents[1] / "bin"))
import importlib.util
spec = importlib.util.spec_from_file_location("cockpit", Path(__file__).resolve().parents[1] / "bin" / "cockpit")
...
Run: `python3 .agent-cockpit/tests/test_wake.py -v`
Expected: FAIL (`AttributeError: module 'cockpit' has no attribute 'resolve_pane'`)

**Step 3: Write minimal implementation** (add near the top of `cockpit`, after imports)
```python
def resolve_pane(panes_text: str, role: str) -> Optional[str]:
    """Map a canonical role to a tmux pane id, given `list-panes -F '#{pane_id}\t#{@cockpit_role}'` output."""
...
**Step 5: Commit**
```bash
git add .agent-cockpit/bin/cockpit .agent-cockpit/tests/test_wake.py
...
**Files:**
- Modify: `.agent-cockpit/bin/cockpit` — add `wake_pane(session, role, text)`
...
**Step 2: Syntax check**
Run: `python3 -m py_compile .agent-cockpit/bin/cockpit`
Expected: no output.

**Step 3: Commit**
```bash
git add .agent-cockpit/bin/cockpit && git commit -m "feat(wake): tmux send-keys wrapper targeting @cockpit_role"
```
...
**Files:**
- Modify: `.agent-cockpit/bin/cockpit` — wrap `append_event` write and `make_id` read-modify-write in `fcntl.flock`
- Test: `.agent-cockpit/tests/test_concurrency.py`
...
```python
# .agent-cockpit/tests/test_concurrency.py
...
class ConcurrencyTests(unittest.TestCase):
    def test_parallel_appends_unique_ids(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            def one(i):
                ev = {"type": "dispatch.sent", "dispatch_id": cockpit.make_id(home, "dsp", "dispatch")}
...
**Step 2: Run test to verify it fails**
Run: `python3 .agent-cockpit/tests/test_concurrency.py -v`
...
**Step 5: Commit**
```bash
git add .agent-cockpit/bin/cockpit .agent-cockpit/tests/test_concurrency.py
...
**Files:**
- Modify: `.agent-cockpit/bin/cockpit` — remove `cmd_run`, `cmd_task`, `cmd_report`, `reconstruct_tasks`, `current_run*`; rebuild `cmd_dispatch`
- Modify: `.agent-cockpit/tests/test_cockpit.py` — drop run/task/report tests; add dispatch tests
- Delete: `.agent-cockpit/bin/{run,task,report,mail}`; Create: `.agent-cockpit/bin/dispatch`
...
**Step 1: Write failing tests** (in `test_cockpit.py`, replacing removed ones)
```python
...
Run: `python3 .agent-cockpit/tests/test_cockpit.py -v`
Expected: FAIL (unknown command `dispatch`, plus removed-test references error)

**Step 3: Implement** — rewrite `cmd_dispatch` (adapt the old `cmd_mail`): build the event with fields `{type:"dispatch.sent", dispatch_id, project, task, from, to, kind, subject, body, reply_to, created_at}`, append under kind `"dispatch"`, mirror to `conversation.log`, and unless `--no-wake`, call `wake_pane(os.environ.get("COCKPIT_TMUX_SESSION",""), to, f"[dispatch from {from_}] {subject or body[:80]} — run ./.agent-cockpit/bin/dispatch inbox {to}")`. Remove run/task/report parsers + handlers from `build_parser`/`main`. Delete obsolete wrappers; create `bin/dispatch` mirroring the `bin/mail` one-liner but calling `dispatch`.

**Step 4: Run tests**
Run: `python3 .agent-cockpit/tests/test_cockpit.py && python3 .agent-cockpit/tests/test_wake.py && python3 .agent-cockpit/tests/test_concurrency.py`
Expected: all PASS

**Step 5: Clean up obsolete state + wrappers**
```bash
cd /Users/seanchiu/Desktop/workspace-macbook
rm -f .agent-cockpit/bin/run .agent-cockpit/bin/task .agent-cockpit/bin/report .agent-cockpit/bin/mail
rm -f .agent-cockpit/state/runs.jsonl .agent-cockpit/state/tasks.jsonl .agent-cockpit/state/reports.jsonl .agent-cockpit/state/current_run.json
```
...
**Files:**
- Modify: `.agent-cockpit/bin/status` (no more `run summary`), `.agent-cockpit/bin/ask`, `.agent-cockpit/bin/start-webapp`
- Delete: `.agent-cockpit/bin/ask` if redundant with `dispatch send` (decide)

**Step 1:** `status` → print a dispatch-log summary instead of run summary:
```bash
exec python3 "$DIR/cockpit" dispatch log "${1:-40}"
```
**Step 2:** `ask` → make it a thin alias for `dispatch send --from ${COCKPIT_FROM:-hermes} --to $1` (wake included by default), or delete it and standardize on `dispatch`. Recommended: delete `ask`; the dispatch verb now covers it.
...
**Step 4: Verify**
```bash
bash -n .agent-cockpit/bin/start-webapp .agent-cockpit/bin/status .agent-cockpit/bin/convo
```
Expected: clean.
...
**Files:**
- Create: `.agent-cockpit/roles/_PROTOCOL.md` (shared protocol — common interaction rules)
- Rewrite: `.agent-cockpit/roles/{orchestrator,frontend-lead,backend-lead,qa-lead}.md` as **thin charters** (4 sections: identity+domain, editable territory & soft guardrails, domain conventions, pointer to `_PROTOCOL.md`)
...
**Step 4: Commit**
```bash
git add .agent-cockpit/roles && git commit -m "docs(roles): thin charters + shared protocol per ADR 0001/0002"
```
...
**Files:**
- Rewrite: `.agent-cockpit/{README,INSTRUCTIONS,HANDOFF}.md`, `ORCHESTRATION.md`, `AGENTS.md`
...
```bash
cd /Users/seanchiu/Desktop/workspace-macbook
grep -rniE '\b(mail|\brun start\b|tasks?\.jsonl|specialist)\b' .agent-cockpit/*.md *.md .agent-cockpit/roles || echo "clean"
```
...
**Why:** Orchestrator + leads must reach Linear. The cockpit doesn't call Linear; the *panes'* Claude Code must have the Linear MCP. Sean previously did not want `.mcp.json` in the repo — so prefer user-level config.
...
**Step 2:** Relaunch: `./.agent-cockpit/bin/start-webapp`

**Step 3:** Confirm panes carry machine roles:
```bash
tmux list-panes -t macbook-webapp:roles -F '#{@cockpit_role}'
```
Expected: orchestrator / frontend-lead / backend-lead / qa-lead

**Step 4: Drive the loop.** From Hermes:
```bash
./.agent-cockpit/bin/dispatch send --from hermes --to orchestrator --subject "smoke" "Create a Linear issue for a trivial task, dispatch backend-lead to do it, and report back."
```
...
**Step 5:** If a wake doesn't land, debug `wake_pane` against live `@cockpit_role` output (the usual cause is a session-name/`COCKPIT_TMUX_SESSION` mismatch).
