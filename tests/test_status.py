...
_JUMPCODE = ROOT / ".jumpcode" / "bin" / "jumpcode"
...
class OpenRequestsTests(unittest.TestCase):
    def test_request_with_no_report_is_open(self):
        ...
        opens = jumpcode.open_requests(d)
    ...
    def test_precise_reply_to_closes_the_loop(self):
        ...
        self.assertEqual(jumpcode.open_requests(d), [])
    ...
    def test_report_from_other_role_does_not_close(self):
        ...
        self.assertEqual([r["dispatch_id"] for r in jumpcode.open_requests(d)], ["dsp-1"])
    ...
    def test_precise_reply_to_wins_over_fallback_steal(self):
        ...
        self.assertEqual([r["dispatch_id"] for r in jumpcode.open_requests(d)], ["dsp-A"])
...
class StatusCliTests(unittest.TestCase):
    ...
    def _run(self, *args, check=True):
        env = os.environ.copy()
        env["JUMPCODE_HOME"] = str(self.tmp_path / ".jumpcode")
        env.pop("JUMPCODE_TMUX_SESSION", None)
        env.pop("TMUX", None)
        r = subprocess.run([sys.executable, str(_JUMPCODE), *args],
